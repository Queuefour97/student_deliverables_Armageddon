...
resource "aws_security_group" "armageddon_alb_sg01" {
  ...
  egress {
    ...
    to_port     = 0
    protocol    = "-1"
    #cidr_blocks = [aws_vpc.armageddon_vpc01.cidr_block]
    # Private CIDR
...
resource "aws_security_group_rule" "armageddon_ec2_ingress_from_alb01" {
  ...
  # Note: This rule assumes the EC2 security group is defined in aws_security_group.armageddon_ec2_sg01
...
resource "aws_acm_certificate" "armageddon_acm_cert01" {
  ...
  domain_name       = local.armageddon_app_fqdn #var.domain_name
...
resource "aws_acm_certificate" "armageddon-final_acm_cert01" {
  domain_name       = "firstpointand.click"
