...
resource "aws_route53_record" "armageddon_cf_validation" {
  for_each = {
...
resource "aws_acm_certificate_validation" "armageddon_cf_validation01" {
  provider                = aws.east
  certificate_arn         = aws_acm_certificate.armageddon_cf_cert01.arn
  validation_record_fqdns = [for r in aws_route53_record.armageddon_cf_validation : r.fqdn]
}