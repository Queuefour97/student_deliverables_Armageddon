...
resource "aws_security_group" "shinjuku_alb_sg01" {
  ...
  egress {
    ...
    to_port     = 0
    protocol    = "-1"
    #cidr_blocks = [aws_vpc.shinjuku_vpc01.cidr_block]
    # Private CIDR
...
resource "aws_security_group_rule" "shinjuku_ec2_ingress_from_alb01" {
  ...
  # Note: This rule assumes the EC2 security group is defined in aws_security_group.shinjuku_ec2_sg01
...
resource "aws_acm_certificate" "shinjuku_acm_cert01" {
  ...
  domain_name       = local.shinjuku_app_fqdn #var.domain_name
...
# Duplicate ACM cert removed — shinjuku_acm_cert01 above covers firstpointand.click
...
resource "aws_lb_listener" "shinjuku_https_listener01" {
  ...
  default_action {
    # 02/19/2026: Changed from forward to fixed-response to block all traffic by default; specific allow rule added in aws_lb_listener_rule.shinjuku_require_origin_header01
    #type             = "forward"
    #target_group_arn = aws_lb_target_group.shinjuku_tg01.arn
    # adding a default fixed-response to block all traffic that doesn't match the specific allow rule with the secret header
    # 221-225: Added aws_lb_listener_rule.shinjuku_default_block01 to handle this instead, so we can return a custom 403 message
    type = "fixed-response"
    fixed_response {
  ...
  depends_on = [aws_acm_certificate_validation.shinjuku_acm_validation01_dns_bonus]
}
