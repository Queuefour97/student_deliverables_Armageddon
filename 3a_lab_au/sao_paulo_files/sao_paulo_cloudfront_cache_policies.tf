...
resource "aws_cloudfront_cache_policy" "liberdale_cache_static01" {
  ...
  parameters_in_cache_key_and_forwarded_to_origin {
    # Explanation: Static should not vary on cookies—armageddon refuses to cache 10,000 versions of a PNG.
...
# # # Explanation: APIs are dangerous to cache by accident—armageddon disables caching until proven safe.
resource "aws_cloudfront_cache_policy" "liberdale_cache_api_disabled01" {
...
# # # Explanation: Origins need context—armageddon forwards what the app needs without polluting the cache key.
resource "aws_cloudfront_origin_request_policy" "liberdale_orp_api01" {
...
# # # Explanation: Static origins need almost nothing—armageddon forwards minimal values for maximum cache sanity.
resource "aws_cloudfront_origin_request_policy" "liberdale_orp_static01" {
...
# # # Explanation: Make caching intent explicit—armageddon stamps Cache-Control so humans and CDNs agree.
resource "aws_cloudfront_response_headers_policy" "liberdale_rsp_static01" {
...
# # Explanation: Default behavior is conservative—armageddon assumes dynamic until proven static.
...
# # # Explanation: Static behavior is the speed lane—armageddon caches it hard for performance.
